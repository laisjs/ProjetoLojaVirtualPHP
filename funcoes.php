<?php
session_start(); //td vez q eu quiser colocar uma informação de um usuário dentro da sessão, é só chamar a função logar usuário
function logarUsuario($nome, $nivelDeAcesso){
   $usuario = ["logado"=>true,"nome"=>$nome, "nivelAcesso" => $nivelDeAcesso];
   $_SESSION['usuario'] = $usuario;
  //como isso vai ta dentro da session não precisam retornar nada. A gente tinah uam função logar usuário 
  //e setava numa array, agora continua construindo a rray, e depois de pronta a gente vai setar essa array
  //dentro de usuário
}
